...
from app.services.sociedad import get_all_sociedades, get_sociedad_by_id, get_sociedad_by_porcentaje, create_sociedad, update_sociedad, delete_sociedad
...
def create_new_sociedad(sociedad: SociedadCreate, db: Session = Depends(get_db)):
    ...
    return create_sociedad(db, sociedad)

@router.put("/{sociedad_id}", response_model=SociedadResponse)
def update_some_sociedad(sociedad_id: int, sociedad: SociedadCreate, db: Session = Depends(get_db)):
    db_sociedad = get_sociedad_by_id(db, sociedad_id)
    if not db_sociedad:
        raise HTTPException(status_code=404, detail="Sociedad no encontrada")
    return update_sociedad(db, sociedad_id, sociedad)

@router.delete("/{sociedad_id}", response_model=SociedadResponse)
def remove_sociedad(sociedad_id: int, db: Session = Depends(get_db)):
    db_sociedad = get_sociedad_by_id(db, sociedad_id)
    if not db_sociedad:
        raise HTTPException(status_code=404, detail="Sociedad no encontrada")
    return delete_sociedad(db, sociedad_id)