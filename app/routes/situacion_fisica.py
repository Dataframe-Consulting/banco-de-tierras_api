...
from app.services.situacion_fisica import get_all_situaciones_fisicas, get_situacion_fisica_by_id, get_situacion_fisica_by_nombre, create_situacion_fisica, update_situacion_fisica, delete_situacion_fisica
...
def create_new_situacion_fisica(situacion_fisica: SituacionFisicaCreate, db: Session = Depends(get_db)):
    ...
    return create_situacion_fisica(db, situacion_fisica)

@router.put("/{situacion_fisica_id}", response_model=SituacionFisicaResponse)
def update_some_situacion_fisica(situacion_fisica_id: int, situacion_fisica: SituacionFisicaCreate, db: Session = Depends(get_db)):
    db_situacion_fisica = get_situacion_fisica_by_id(db, situacion_fisica_id)
    if not db_situacion_fisica:
        raise HTTPException(status_code=404, detail="Situación Física no encontrada")
    return update_situacion_fisica(db, situacion_fisica_id, situacion_fisica)

@router.delete("/{situacion_fisica_id}", response_model=SituacionFisicaResponse)
def remove_situacion_fisica(situacion_fisica_id: int, db: Session = Depends(get_db)):
    db_situacion_fisica = get_situacion_fisica_by_id(db, situacion_fisica_id)
    if not db_situacion_fisica:
        raise HTTPException(status_code=404, detail="Situación Física no encontrada")
    return delete_situacion_fisica(db, situacion_fisica_id)